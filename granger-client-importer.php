...
class GrangerClientImporter {
    ...
    public function render_admin_page() {
        ...
        <div class="wrap">
            ...
            <style>
            .gci-banner{background:#F6FAFE;border:1px solid #D6E8FB;padding:12px 14px;border-radius:8px;margin:12px 0;color:#1d2327}
            .gci-grid{display:grid;grid-template-columns:1fr;gap:16px}
            .card.gci-card{border:1px solid #dcdfe4;box-shadow:0 1px 2px rgba(0,0,0,0.04);border-radius:10px}
            .gci-card h2{margin-top:0}
            .gci-badge{background:#2271b1;color:#fff;border-radius:999px;padding:2px 8px;font-size:12px}
            .gci-actions{display:flex;gap:10px;align-items:center}
            .gci-progress{width:100%;background:#f0f2f4;border:1px solid #dcdfe4;border-radius:6px;overflow:hidden}
            .gci-progress-fill{height:14px;background:#2271b1;width:0%}
            #gci-log{font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace}
            .gci-footer-note{color:#666;font-size:12px;margin-top:6px}
            .gci-success{border-left:4px solid #46b450;background:#f4fbf6;padding:10px;border-radius:6px}
            .gci-error{border-left:4px solid #d63638;background:#fff5f5;padding:10px;border-radius:6px}
            </style>

            <div class="gci-banner">
                <strong>Tips:</strong> CSV loads fastest. Start with a moderate batch (100–300). If you enable reset emails, make sure outgoing mail (SMTP) is configured.
            </div>
            ...
            <div class="gci-grid">
                        ...
                        <table class="form-table">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <p class="description">First row must contain headers. Core columns detected by smart aliases (Email, Username/Login, First/Last Name, Display Name, Role, Password).</p>
                                </td>
                            </tr>
                        </table>
                    </form>
                ...
                <div class="card gci-card" id="gci-import-section" style="display: <?php echo $saved_path ? 'block' : 'none'; ?>;">
                    ...
                    <form id="gci-import-form">
                        <table class="form-table">
                            ...
                            <tr>
                                ...
                                <td>
                                    <label><input type="checkbox" id="gci-update-existing" checked> Update existing users (match by Email or Username)</label>
                                </td>
                            </tr>
                            <tr>
                                <th>Role</th>
                                <td>
                                    <div class="gci-actions">
                                        ...
                                        <label><input type="checkbox" id="gci-force-subscriber" checked> Force Subscriber for all</label>
                                    </div>
                                    <p class="description">Subscriber is recommended for site access without admin privileges.</p>
                                </td>
                            </tr>
                            <tr>
                                <th>Passwords</th>
                            ...
                            <tr>
                                <th>Batch Size</th>
                                <td>
                                    <input type="number" id="gci-batch-size" value="200" min="1" max="5000" style="width:100px;">
                                    <p class="description">Higher is faster but uses more memory/CPU.</p>
                                </td>
                            </tr>
                        </table>
                    ...
                    <div id="gci-progress" style="display:none;">
                        ...
                        <div id="gci-log" class="gci-log"></div>
        ...
        jQuery(function($){
            // Step 1 upload
            $('#gci-upload-form').on('submit', function(e){
                e.preventDefault();
                var fd = new FormData();
                fd.append('action','gci_upload_file');
                fd.append('nonce','<?php echo wp_create_nonce('gci_upload'); ?>');
                fd.append('gci_file', $('input[name="gci_file"]')[0].files[0]);
            ...
            $('#gci-import-form').on('submit', function(e){
                ...
                const batchSize = parseInt($('#gci-batch-size').val(),10)||200;
                ...
                const forceSubscriber = $('#gci-force-subscriber').is(':checked') ? 1 : 0;
                function tick(){
                    if(stopped) return;
                    $.post(ajaxurl, {action:'gci_start_import', nonce:'<?php echo wp_create_nonce('gci_import'); ?>', offset, batch_size: batchSize, update_existing:updateExisting, default_role:defRole, passwords_hashed: hashedPw, send_reset: sendReset, force_subscriber: forceSubscriber}, function(res){
                        if(res.success){
                            ...
                            $('#gci-progress-text').text('Imported '+offset+' of '+total);
                            if(res.data.log && res.data.log.length){ res.data.log.forEach(line=>$('#gci-log').append('<div>'+line+'</div>')); }
                            ...
                            $('#gci-log').append('<div class="gci-error">'+res.data+'</div>');
                        }
                    }).fail(function(){ $('#gci-log').append('<div class="gci-error">Server busy, retrying…</div>'); setTimeout(tick, 1500); });
                }
                tick();
            });
        });
    ...
    public function ajax_start_import() {
        ...
        $forceSubscriber = !empty($_POST['force_subscriber']);
        ...
        $processed = 0; $log = [];
        foreach ($rows as $row) {
            $res = $this->import_single_user($row, $updateExisting, $defaultRole, $batchId, $passwordsHashed, $sendReset, $forceSubscriber);
    ...
    private function import_single_user($row, $updateExisting, $defaultRole, $batchId, $passwordsHashed, $sendReset, $forceSubscriber = false) {
        ...
        if ($user) {
            ...
            if ($updateExisting) {
                ...
                if ($forceSubscriber) { $u=new WP_User($user->ID); $u->set_role('subscriber'); }
                elseif ($role && !in_array($role, $user->roles, true)) { $u=new WP_User($user->ID); $u->set_role($role); }
                if (!$passwordsHashed && !empty($password)) { wp_set_password($password, $user->ID); }
                if ($passwordsHashed && !empty($password)) { update_user_meta($user->ID, 'gci_password_hash', $password); if ($sendReset) { $this->send_reset_email($user->ID); } }
                $this->save_user_meta_bulk($user->ID, $normalized, $batchId);
                return ['ok'=>true,'message'=>'Updated: '.$user->user_login];
            } else {
        ...
        if ($passwordsHashed) {
            ...
            if ($user_url) { $insert['user_url'] = esc_url_raw($user_url); }
            $new_user_id = wp_insert_user($insert);
            if (is_wp_error($new_user_id)) { return ['ok'=>false,'message'=>'Failed: '.$new_user_id->get_error_message()]; }
            if (!empty($password)) { update_user_meta($new_user_id, 'gci_password_hash', $password); }
            if ($sendReset) { $this->send_reset_email($new_user_id); }
        } else {
        ...
        $this->save_user_meta_bulk($new_user_id, $normalized, $batchId);
