...
class GrangerClientImporter {
    ...
    public function render_admin_page() {
        ...
        <div class="wrap">
            ...
            .gci-banner{background:#F6FAFE;border:1px solid #D6E8FB;padding:12px 14px;border-radius:8px;margin:12px 0;color:#1d2327;width:100%;box-sizing:border-box}
            .gci-grid{display:grid;grid-template-columns:1fr;gap:16px;width:100%}
            .card.gci-card{border:1px solid #dcdfe4;box-shadow:0 1px 2px rgba(0,0,0,0.04);border-radius:10px;width:100%;box-sizing:border-box}
            .gci-card h2{margin-top:0}
            .gci-badge{background:#2271b1;color:#fff;border-radius:999px;padding:2px 8px;font-size:12px}
            .gci-actions{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
            ...
            #gci-log, #gci-added{font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace}
            ...
            .gci-contact{border:1px dashed #dcdfe4;padding:12px;border-radius:10px;background:#fcfcfc}
            .gci-panels{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:10px}
            .gci-panel{border:1px solid #e5e7eb;border-radius:8px;background:#fff;padding:8px;box-sizing:border-box}
            .gci-panel h3{margin:4px 6px 8px;font-size:14px}
            .gci-scroll{max-height:260px;overflow:auto;padding:6px;border-radius:6px;background:#fafafa}
            .form-table{width:100%}
            .form-table th,.form-table td{padding-right:12px}
            @media (max-width: 900px){ .gci-panels{grid-template-columns:1fr} }
            @media (min-width:1400px){ .gci-grid{grid-template-columns:1fr} }
            </style>
        ...
        jQuery(function($){
            ...
            $('#gci-import-form').on('submit', function(e){
                ...
                function tick(){
                    if(stopped) return;
                    $.post(ajaxurl, {action:'gci_start_import', nonce:'<?php echo wp_create_nonce('gci_import'); ?>', offset, batch_size: currBatch, update_existing:updateExisting, strict_skip: strictSkip, default_role:defRole, passwords_hashed: hashedPw, send_reset: sendReset, force_subscriber: forceSubscriber}, function(res){
                        if(res.success){
    ...
    public function ajax_start_import() {
        ...
        $processed = 0; $log = [];
        foreach ($rows as $row) {
            $res = $this->import_single_user($row, $updateExisting && !$strictSkip, $defaultRole, $batchId, $passwordsHashed, $sendReset, $forceSubscriber);
